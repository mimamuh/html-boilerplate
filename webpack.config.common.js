...
function defaultValue(options, optionKey, defaultVal) {
    return (Object.prototype.hasOwnProperty.call(options, optionKey)) ? options[optionKey] : defaultVal;
}
...
function useHtmlTemplateFactory(globalOptions) {
    ...
    if (googleAnalytics) {
        googleAnalytics = {
            ...
            trackingId: defaultValue(googleAnalytics, 'trackingId', 'UA-XXXX-XX'),
            // Log a pageview event after the analytics code loads.
            pageViewOnLoad: defaultValue(googleAnalytics, 'pageViewOnLoad', true),
    ...
    globalOptions = {
        ...
        baseHref: defaultValue(globalOptions, 'baseHref', 'http://localhost:8080'),
        // insert the webpack-dev-server hot reload script
        // at this host:port/path; e.g., http://localhost:3000.
        devServer: defaultValue(globalOptions, 'devServer', 'http://localhost:8080'),
        // Track usage of your site via Google Analytics.
        googleAnalytics,
    ...
    return (options) => {
        console.log(path.resolve(__dirname, globalOptions.outputPath, path.parse(options.template).base));
